...
class PluginInstanceTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        session_id = queue_net_in = queue_net_out = proc_pipe = \
            serializer = plugin_id = version_table = original_version_table = \
            permissions = MagicMock()
        self.plugin_instance._setup_networking(
            session_id, queue_net_in, queue_net_out, proc_pipe, serializer,
            plugin_id, version_table, original_version_table, self.custom_data,
            permissions)
